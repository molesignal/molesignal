...
//! `agent_model_providers` + `agent_model_provider_secrets` 表 Pg 实装。
//!
//! Provider 元数据 org-scoped；API key 明文经 [`CipherRootKey`] AES-256-GCM `seal`
//! 后存到 `agent_model_provider_secrets`（密文 + nonce），元数据表只留 masked
...
impl PgModelProviderRepository {
    ...
    async fn seal_secret(&self, org_id: &Id, provider_id: &Id, api_key: &str) -> Result<()> {
        ...
        sqlx::query(
            "INSERT INTO agent_model_provider_secrets
...
impl ModelProviderRepository for PgModelProviderRepository {
    async fn list(&self, org_id: &Id) -> Result<Vec<ModelProvider>> {
        let rows = sqlx::query(&format!(
            "SELECT {COLS} FROM agent_model_providers
    ...
    async fn get(&self, org_id: &Id, id: &Id) -> Result<ModelProvider> {
        let row = sqlx::query(&format!(
            "SELECT {COLS} FROM agent_model_providers WHERE org_id = $1 AND id = $2"
        ))
        .bind(&org_id.0)
        .bind(&id.0)
    ...
    async fn update(&self, input: ModelProviderInput) -> Result<ModelProvider> {
        let now = TimestampMicros::now();
        let res = sqlx::query(
            "UPDATE agent_model_providers SET
    ...
    async fn rotate_key(&self, org_id: &Id, id: &Id, api_key: &str) -> Result<ModelProvider> {
        ...
        let now = TimestampMicros::now();
        sqlx::query(
            "UPDATE agent_model_providers SET key_last4 = $3, key_set = TRUE, updated_at_micros = $4
             WHERE org_id = $1 AND id = $2",
        )
        .bind(&org_id.0)
        .bind(&id.0)
    ...
    async fn delete(&self, org_id: &Id, id: &Id) -> Result<()> {
        sqlx::query(
            "DELETE FROM agent_model_provider_secrets WHERE provider_id = $1 AND org_id = $2",
        )
        .bind(&id.0)
        .bind(&org_id.0)
        .execute(&self.pool)
        .await
        .map_err(sqlx_err)?;
        sqlx::query("DELETE FROM agent_model_providers WHERE org_id = $1 AND id = $2")
            .bind(&org_id.0)
            .bind(&id.0)
            .execute(&self.pool)
            .await
    ...
    async fn get_plaintext_key(&self, org_id: &Id, id: &Id) -> Result<Option<String>> {
        let row = sqlx::query(
            "SELECT ciphertext, nonce FROM agent_model_provider_secrets
