...
const fn view_predicate(view: StatusPageEventView) -> &'static str {
    match view {
        ...
        StatusPageEventView::Draft => {
            "publication_state = 'draft' AND NOT EXISTS (
                SELECT 1 FROM status_page_automation_candidates candidate
                WHERE candidate.organization_id = status_page_incidents.org_id
                  AND candidate.status_page_id = status_page_incidents.status_page_id
                  AND candidate.status_incident_id = status_page_incidents.id
                  AND candidate.state IN ('rejected', 'cancelled', 'resolved')
            )"
        }
