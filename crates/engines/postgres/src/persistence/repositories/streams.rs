...
use crate::{
    domain::stream::{
        MOLESIGNAL_SYSTEM_STREAM, Retention, Schema, StreamDefinition, StreamMutationObserver,
        StreamRepository, StreamSettings, StreamType,
    },
...
pub struct PgStreamRepository {
    pool: PgPool,
    /// Optional projection hook notified after committed stream mutations.
    mutation_observer: Option<Arc<dyn StreamMutationObserver>>,
}
...
impl PgStreamRepository {
    ...
    /// Injects the runtime projection observer; tests can continue using [`Self::new`].
    pub fn with_mutation_observer(mut self, observer: Arc<dyn StreamMutationObserver>) -> Self {
...
fn invalidate_stream_cache(
    observer: &Option<Arc<dyn StreamMutationObserver>>,
    org_id: &str,
    name: &str,
    stream_type: &str,
) {
    ...
    {
        observer.stream_changed(&Id(org_id.to_string()), name, stream_type);
    }
}
...
impl StreamRepository for PgStreamRepository {
    ...
    async fn update_schema(&self, id: &Id, schema: Schema) -> Result<()> {
        ...
        invalidate_stream_cache(&self.mutation_observer, &org_id, &name, &st);
        Ok(())
    }
    ...
    async fn update_schema_internal(&self, id: &Id, schema: Schema) -> Result<()> {
        ...
        invalidate_stream_cache(&self.mutation_observer, &org_id, &name, &stream_type);
        Ok(())
    }
