...
const SPECS: &[RewrapSpec] = &[
    ...
    RewrapSpec {
        table: "synthetic_secret_versions",
        pk: &["secret_id", "version"],
        pairs: &[("nonce", "ciphertext")],
    },
    RewrapSpec {
        table: "synthetic_probe_certificate_authorities",
        pk: &["id"],
        pairs: &[("private_key_nonce", "private_key_ciphertext")],
    },
];
...
mod tests {
    ...
    fn update_sql_numbers_set_then_where() {
        ...
        assert_eq!(
            sql,
            "UPDATE cluster_secrets SET nonce = $1, ciphertext = $2 WHERE org_id::TEXT = $3 AND ref_id::TEXT = $4"
        );
    }
    ...
    fn specs_cover_all_known_sealed_tables() {
        ...
        for t in [
            ...
            "notify_connectors",
            "synthetic_secret_versions",
            "synthetic_probe_certificate_authorities",
        ] {
