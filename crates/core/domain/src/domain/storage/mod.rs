...
/// Parsed dump rows cache used by the cold-query path.
///
/// The cache key and eviction policy remain infrastructure details; the PostgreSQL repository
/// only identifies a dump through its persisted index row.
#[async_trait]
pub trait ParquetFileMetaDumpCache: Send + Sync {
    async fn get(&self, dump: &ParquetFileMetaDumpRow) -> Option<Arc<Vec<ParquetFileMeta>>>;

    async fn insert(&self, dump: &ParquetFileMetaDumpRow, value: Arc<Vec<ParquetFileMeta>>);
}

/// Loads and decodes a ParquetFileMeta dump without coupling persistence to object storage.
#[async_trait]
pub trait ParquetFileMetaDumpReader: Send + Sync {
    async fn read(&self, object_key: &str) -> Result<Vec<ParquetFileMeta>>;

    /// Records that a query produced rows from the cold dump tier.
    fn record_query_hit(&self);
}
