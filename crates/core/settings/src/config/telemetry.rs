...
impl TraceSettings {
    ...
    pub fn runtime_policy(&self) -> crate::shared::tail_sampling::TraceRuntimePolicy {
        use crate::shared::tail_sampling::{SlowThresholds, TraceRuntimePolicy};

        TraceRuntimePolicy {
            version: 1,
            enabled: self.effective_enabled(),
            normal_sample_ratio: self.effective_normal_sample_ratio(),
            decision_window_ms: self.decision_window_secs.saturating_mul(1_000),
            root_grace_ms: self.root_grace_millis,
            decision_cache_ms: self.decision_cache_secs.saturating_mul(1_000),
            max_traces: self.tail_max_traces,
            memory_bytes: self.tail_memory_bytes,
            max_spans_per_trace: self.max_spans_per_trace,
            slow_thresholds_ms: SlowThresholds {
                http: self.slow_thresholds.http_ms,
                query: self.slow_thresholds.query_ms,
                batch_intake: self.slow_thresholds.batch_intake_ms,
                database: self.slow_thresholds.database_ms,
                object_store: self.slow_thresholds.object_store_ms,
                external: self.slow_thresholds.external_ms,
                background: self.slow_thresholds.background_ms,
                other: self.slow_thresholds.http_ms,
            },
