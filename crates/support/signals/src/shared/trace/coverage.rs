...
pub const WORKER_COVERAGE: &[WorkerCoverage] = &[
    ...
    WorkerCoverage {
        source: "src/bootstrap/workers/synthetics.rs",
        policy: WorkerTracePolicy::RootPerRun,
    },
    WorkerCoverage {
...
pub const HTTP_CLIENT_COVERAGE: &[&str] = &[
    ...
    "crates/modules/agent/src/chat/providers/anthropic.rs",
    "crates/modules/agent/src/chat/providers/openai.rs",
];
...
mod tests {
    ...
    fn root() -> PathBuf {
        workspace_root().join("bin/molesignal")
    }

    fn read(relative: &str) -> String {
        let app_source = root().join(relative);
        let path = if app_source.exists() {
            app_source
        } else {
    ...
    fn rust_files_with_direct_transaction_begin(path: &Path, out: &mut BTreeSet<String>) {
        ...
        for entry in fs::read_dir(path).expect("read Rust source directory") {
            ...
            if path.is_dir() {
                ...
                && path != workspace_root().join("crates/support/sqlx-shim/src/lib.rs")
                && fs::read_to_string(&path)
    ...
    fn sql_object_store_and_outbound_clients_cannot_bypass_shared_boundaries() {
        let sql = fs::read_to_string(workspace_root().join("crates/support/sqlx-shim/src/lib.rs"))
            .expect("read traced SQLx facade");
        for wrapper in [
            "pub fn query(",
            "pub fn query_as<",
            "pub fn query_scalar<",
        ...
        assert_eq!(
            read("crates/engines/postgres/src/persistence/pool.rs")
