...
The system SHALL hold all `signing_secrets` rows where `kind='jwt'` AND `retired_at_micros IS NULL OR retired_at_micros > now - 24h` in memory as the active secret set. `IdentityService::verify_token` SHALL attempt each active secret in order (primary first); the first successful decode wins.
