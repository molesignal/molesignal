...
### Requirement: PromQL Builder Route

The web app SHALL expose `/metrics/promql-builder` as a PromQL query-builder UI on top of the existing metrics endpoint. The builder SHALL let the user pick a metric name, add label matchers, and pick an aggregation function before pressing `Run`; pressing `Run` issues a `POST /api/v1/metrics/query` and renders the result via the existing timeseries primitive.

#### Scenario: Run executes the composed query

- **WHEN** the user picks metric `http_requests_total`, label `status="200"`, function `rate()`, range `5m`, and clicks Run
...
### Requirement: Actions Route

The web app SHALL expose `/actions` listing configured workflow actions from the enterprise actions endpoint. On the OSS build, the page SHALL read the `/api/v1/license` snapshot and render a `License-gated` empty state with copy explaining that Actions require an enterprise license when `features` does not include `actions`.

#### Scenario: Enterprise build lists actions

- **WHEN** the user opens `/actions` with an enterprise license that includes `actions`
- **THEN** the page renders the actions list returned by `/api/v1/actions`

#### Scenario: OSS build shows license-gated state

- **WHEN** the user opens `/actions` on the OSS build (license features does not include `actions`)
- **THEN** the page renders an `EmptyState` with `Actions require an enterprise license` and no 403 is shown
