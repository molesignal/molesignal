...
### Requirement: Enrichment Table Lookup

The system SHALL support stream_type `enrichment`; rows in such streams have schema `(key TEXT, value JSONB)`. On ingester startup (and on enrichment-stream update), the table is materialized into an in-memory `HashMap<String, serde_json::Value>` keyed by `key`. A VRL built-in function `lookup(table_name, key)` SHALL return the matched value (or `null`) for use in pipeline steps.

#### Scenario: Create enrichment table
- **WHEN** an Editor POSTs `POST /api/v1/streams { name: "geo_ip", stream_type: "enrichment", schema: [{name:"key",type:"Utf8"},{name:"value",type:"Json"}] }` then ingests rows via `/api/v1/ingest/enrichment/geo_ip`
- **THEN** the rows are stored and the in-memory map is refreshed within `enrichment.refresh_interval_secs` (default 60)

#### Scenario: VRL lookup hits enrichment
...
- **THEN** the run aborts with `last_error` recorded; the next cron tick attempts again from the current window (no replay backfill in this MVP)
