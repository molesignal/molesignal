...
The web app SHALL expose `/pipelines/add` and `/pipelines/:id/edit` for creating and editing pipelines via a form-based editor (sources → VRL transform → sinks). The form persists via `POST /api/v1/scheduled_pipelines` / `PUT /api/v1/scheduled_pipelines/:id`.
