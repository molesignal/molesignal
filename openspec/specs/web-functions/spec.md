# Web Functions Capability

## Purpose

Provides the web UI for managing reusable Vector Remap Language (VRL) functions and extend lookup tables used by pipelines during transform.
...
### Requirement: Extend Tables

The web app SHALL expose `/extend-tables` for managing lookup tables (CSV / TSV) that pipelines can join against during transform. Each row carries `name`, `column_count`, `row_count`, `last_loaded_at`. Upload accepts CSV / TSV with auto-detected schema.

#### Scenario: Upload CSV by URL or paste

- **WHEN** the user opens `/extend-tables/new` and pastes a CSV blob
- **THEN** the page issues `POST /api/v1/extend_tables` with the table name + body
