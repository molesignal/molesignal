# Pipeline Runs Capability

## Purpose

Persistence and read API for scheduled-pipeline execution history: the `pipeline_runs` Postgres table that captures every scheduler tick (running / succeeded / failed / cancelled) and the `GET /scheduled_pipelines/{id}/runs` endpoint that exposes recent history to operators.
