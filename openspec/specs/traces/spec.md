...
OTLP trace 入库后的服务图（service graph）滚动聚合：以分钟为窗口提取相邻 span 对的请求量 / 错误数 / 延迟分位数，供拓扑可视化与延迟下钻使用。
...
> Follow-up: end-to-end integration test `it_service_graph.rs` (task 21.8) requires docker + Postgres testcontainer and is deferred until the e2e staging environment is available.
