# Web Cross-Signal Correlation

## Purpose

Cross-signal jump providers (metric ↔ trace ↔ log ↔ host, 8 pairs) that translate the source row's halo time window + filter fields into a prefilled target frame; server-side helper at /api/v1/web/correlation with 400ms client-side timeout + local fallback.
