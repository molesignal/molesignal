...
export interface IntakeContextOptions {
  isRum: boolean;
  applicationId: string;
}
...
export function useIntakeContext(options: IntakeContextOptions): IntakeContext {
  ...
  const tokenQuery = useQuery({
    queryKey: options.isRum
      ...
      : ['default-intake-token', currentOrgId],
    queryFn: () =>
      options.isRum
...
export function substitute(content: string, context: IntakeContext): string {
  return content
    ...
    .replaceAll('{{TOKEN}}', context.token || 'YOUR_INTAKE_TOKEN');
}
