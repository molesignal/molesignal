...
import * as chatApi from '@/api/agent/chat';
...
import { displayTitleForChat, titleForNewChat } from './presentation';
import { AgentWorkspace } from './workspace/AgentWorkspace';
import { ConversationHistory } from './workspace/ConversationHistory';
import { SystemContext } from './workspace/SystemContext';
import {
  CHAT_MODES,
  EXECUTION_POLICIES,
  type ChatContext,
    ...
    >
      {!embedded && (
        <ConversationHistory
          ...
          streaming={streaming}
          width={chatListWidth}
          resizing={isChatListResizing}
          onNewChat={newChat}
          onSelectChat={selectChat}
          onDeleteChat={setChatPendingDeletion}
          onResizePointerDown={beginChatListResize}
          onResizeKeyDown={handleChatListResizeKeyDown}
          onResetWidth={resetChatListWidth}
        />
      )}
      ...
      <div id="agent-chat-transcript" className="flex min-h-0 min-w-0 flex-1 flex-col">
        {!embedded && (
          <div className="flex h-11 shrink-0 items-center justify-between border-b border-bd-0 bg-bg-1 px-2 md:justify-end xl:hidden">
            <button
              ...
              className="inline-flex min-h-11 items-center gap-2 rounded-md px-2 type-caption font-strong text-tx-1 hover:bg-bg-2 focus-visible:bg-bg-2 md:hidden"
            >
              <MessageSquareText aria-hidden="true" className="h-4 w-4 text-tx-3" />
              {t('chats')}
            </button>
            <button
              type="button"
              onClick={() => setMobileContextOpen(true)}
              className="inline-flex min-h-11 items-center gap-2 rounded-md px-2 type-caption font-strong text-tx-1 hover:bg-bg-2 focus-visible:bg-bg-2"
            >
              <SlidersHorizontal aria-hidden="true" className="h-4 w-4 text-tx-3" />
        ...
        >
          {showEmptyWorkspace ? (
            <div className="mx-auto flex min-h-full w-full max-w-[960px] flex-col gap-3 py-3 md:py-4">
              <AgentWorkspace
      ...
      {!embedded && (
        <>
          <Sheet open={mobileHistoryOpen} onOpenChange={setMobileHistoryOpen}>
            <SheetContent
              side="left"
              className="w-[min(88vw,320px)] border-bd-0 bg-bg-1 p-0 pt-10 shadow-none [&>button]:left-3 [&>button]:right-auto"
            >
          ...
          <Sheet open={mobileContextOpen} onOpenChange={setMobileContextOpen}>
            <SheetContent
              side="right"
              className="w-[min(88vw,320px)] border-bd-0 bg-bg-1 p-0 pt-10 shadow-none"
            >
...
function ChatComposer({
  ...
  selectedAgentProfile: agentApi.AgentProfile | undefined;
  providerId: string;
  ...
  return (
    ...
    >
      ...
      <textarea
        ...
        ref={composerRef}
        className="block max-h-[200px] min-h-11 w-full resize-none overflow-y-hidden border-0 bg-transparent px-3.5 py-2.5 font-sans text-base leading-6 text-tx-0 placeholder:text-tx-3 focus:outline-none md:text-sm"
      />
      <div
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <button
          type="button"
          className="inline-flex h-10 shrink-0 items-center gap-1.5 rounded-md px-2.5 font-sans text-xs font-strong text-tx-2 hover:bg-bg-3 hover:text-tx-0 focus-visible:bg-bg-3 focus-visible:text-tx-0 md:h-8"
        >
          <MoreHorizontal className="h-3.5 w-3.5" />
          {t('advanced.trigger')}
          {hasOverrides && (
            <span
              aria-label={t('advanced.overrides_active')}
    ...
    <Popover>
      <PopoverTrigger asChild>
        <button
          type="button"
          className={cn(
            'inline-flex shrink-0 items-center gap-1.5 rounded-md px-2 font-sans text-xs font-strong text-tx-2 hover:bg-bg-2 hover:text-tx-0 focus-visible:bg-bg-2 focus-visible:text-tx-0',
            triggerClassName,
          )}
        >
          <Plus className="h-3.5 w-3.5" />
...
function InvestigationProcess({
  ...
  return (
    <details data-testid="investigation-process" className="group mt-3">
      <summary className="inline-flex min-h-11 cursor-pointer list-none items-center gap-2 rounded-md px-1.5 font-sans text-xs font-strong text-tx-2 transition-colors duration-fast hover:bg-bg-2 hover:text-tx-0 focus-visible:bg-bg-2 focus-visible:text-tx-0 md:min-h-9 [&::-webkit-details-marker]:hidden">
