...
export type InvestigationEditorTarget =
  | agentApi.Investigation
  | 'new'
  | null;
export type AutomationEditorTarget = agentApi.Automation | 'new' | null;

interface InvestigationDraft {
  title: string;
  status: agentApi.InvestigationStatus;
  summary: string;
  confidence: agentApi.ConfidenceLevel | '';
  currentStep: string;
  context: string;
  steps: string;
}
...
export function InvestigationEditorDrawer({
  ...
  const save = useMutation({
    mutationFn: async ({
      ...
      if (!id) {
        return agentApi.createInvestigation({
          title: draft.title.trim(),
          context,
          steps: parseDelimitedList(draft.steps),
        });
      }
      return agentApi.updateInvestigation(id, {
        title: draft.title.trim(),
        status: draft.status,
    ...
    onSuccess: async (saved, variables) => {
      await Promise.all([
        ...
        queryClient.invalidateQueries({
          queryKey: ['agent', 'investigation', saved.id],
        }),
  ...
  const formId = 'agent-investigation-editor';
  return (
    <FormDrawer
      open={target !== null}
      onOpenChange={(open) => {
                ...
                <FormField label={t('common.status')}>
                  <FormSelect
                    ...
                    onChange={(status) =>
                      setDraft((value) => ({
                        ...value,
                        status: status as agentApi.InvestigationStatus,
                      }))
                    }
                    options={[
                      'draft',
                  ...
                  <FormSelect
                    ...
                    onChange={(confidence) =>
                      setDraft((value) => ({
                        ...value,
                        confidence:
                          confidence as agentApi.ConfidenceLevel | '',
                      }))
                    }
                    options={[
...
export function AutomationEditorDrawer({
  ...
  const save = useMutation({
    mutationFn: ({
      ...
      id: string | null;
      input: agentApi.AutomationInput;
    }) =>
      id
        ? agentApi.updateAutomation(id, input)
        : agentApi.createAutomation(input),
  ...
  const isNew = target === 'new';
  const formId = 'agent-automation-editor';
  return (
    <FormDrawer
      open={target !== null}
      onOpenChange={(open) => {
