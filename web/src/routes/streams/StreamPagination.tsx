...
const STREAM_PAGE_SIZE_OPTIONS = [20, 50, 100];
const DEFAULT_STREAM_PAGE_SIZE = 20;

export function useStreamPagination<T>(
  items: readonly T[],
  resetKey: string,
) {
  const [page, setPage] = React.useState(1);
  const [pageSize, setPageSize] = React.useState(DEFAULT_STREAM_PAGE_SIZE);
...
export function StreamPagination({
  page,
  pageCount,
  pageSize,
  total,
  ...
  if (total <= DEFAULT_STREAM_PAGE_SIZE) return null;
  ...
  return (
    <ResultPagination
      ...
      pageCount={pageCount}
      pageSize={pageSize}
      pageSizeOptions={STREAM_PAGE_SIZE_OPTIONS}
      ...
      className="ml-auto mt-2 w-fit gap-2 border-0 bg-transparent px-0"
    />
  );
}
