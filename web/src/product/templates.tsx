...
export function ManagementPage({
  ...
  const collapsed = useSubnavStore((s) => s.collapsed);
  const toggle = useSubnavStore((s) => s.toggle);
  return (
    <ProductPageFrame
      {...frame}
      bodyClassName={cn(
    ...
    >
        ...
        (collapsed ? (
          ...
          <aside>
            <button
              type="button"
              onClick={toggle}
              aria-label={t('subnav.expand', { defaultValue: 'Expand menu' })}
              title={t('subnav.expand', { defaultValue: 'Expand menu' })}
              className="sticky top-6 grid h-9 w-9 place-items-center rounded-md border border-bd-0 bg-bg-1 text-tx-2 hover:bg-bg-3 hover:text-tx-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo"
            >
          ...
          <aside className="relative min-w-0">
            <button
              type="button"
              onClick={toggle}
              aria-label={t('subnav.collapse', { defaultValue: 'Collapse menu' })}
              title={t('subnav.collapse', { defaultValue: 'Collapse menu' })}
              className="absolute right-2 top-2 z-10 grid h-8 w-8 place-items-center rounded-md text-tx-3 hover:bg-bg-3 hover:text-tx-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo"
            >
      ...
      <div className="min-w-0">{state ? <ProductState {...state} /> : children}</div>
    </ProductPageFrame>
  );
}
