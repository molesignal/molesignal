...
//!
//! 其余场景在别处单测：tool 证据抽取见 `agent::chat::tests::summarize_*`；跨 org tool
//! 隔离见 `agent_mcp_dispatcher::tests::list_streams_uses_ctx_org_and_ignores_arg_org_id`；
...
fn user_msg(s: &str) -> molesignal::agent::chat::ChatMessage {
    molesignal::agent::chat::ChatMessage {
        id: Id::new(),
        chat_id: Id("s".into()),
...
async fn http_chat_prompt_metadata_and_archive() {
    ...
    // 5) 归档 transcript → agent_chat_archives 落一行（status ok + object_key）。
    let archive: serde_json::Value = server
        .client
        .post(format!(
            "{}/api/v1/agent/chat/{}/archive",
            server.base_url, chat_id
        ))
        .header("authorization", format!("Bearer {}", server.root_token))
        .send()
