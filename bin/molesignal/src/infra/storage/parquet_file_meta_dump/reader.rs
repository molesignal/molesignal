...
use crate::{
    domain::{
        storage::{
            ParquetFileMeta, ParquetFileMetaDumpReader as ParquetFileMetaDumpReaderPort,
            PhysicalDatasetKind,
        },
        stream::StreamType,
    },
...
/// Object-store-backed adapter for the cold-query dump reader port.
pub struct ObjectStoreParquetFileMetaDumpReader {
    object_store: Arc<dyn ObjectStore>,
}

impl ObjectStoreParquetFileMetaDumpReader {
...
#[async_trait]
impl ParquetFileMetaDumpReaderPort for ObjectStoreParquetFileMetaDumpReader {
    async fn read(&self, object_key: &str) -> Result<Vec<ParquetFileMeta>> {
        read_dump(self.object_store.clone(), object_key).await
    }

    fn record_query_hit(&self) {
        crate::infra::storage::parquet_file_meta_dump::reader::record_query_hit();
    }
}
