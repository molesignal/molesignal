...
use crate::{
    ...
    infra::intake::metrics::{add_reserved_buffer_bytes, inc_memory_rejection},
    shared::{Error, Result},
};
...
impl MemoryBudget {
    ...
    pub(super) fn try_reserve(
        ...
        if !self.reserve_with_limit(bytes) {
            ...
            return Err(Error::resource_exhausted(
                "intake buffer memory limit exceeded; retry later",
            ));
        }
    ...
    pub(super) fn force_reserve(self: &Arc<Self>, bytes: usize) -> Result<MemoryReservation> {
        self.reserved_bytes
            ...
            .map_err(|_| Error::internal("intake buffer memory accounting overflow"))?;
