...
use crate::{
    domain::stream::{StreamDefinition, StreamMutationObserver, StreamType},
    shared::ids::Id,
};
...
impl StreamMutationObserver for OrgSchemaCache {
    fn stream_changed(&self, org_id: &Id, name: &str, stream_type: StreamType) {
        self.invalidate_for(org_id, name, stream_type);
    }
}
