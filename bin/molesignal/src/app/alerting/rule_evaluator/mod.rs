...
use crate::{
    app::{
        ...
        notify::{ALERT_RESOLVED_EVENT, ALERT_TRIGGERED_EVENT, NotifyEngine},
    },
    domain::{
        alerting::{
...
pub struct RuleEvaluator {
    ...
    lifecycle_sink: Option<Arc<dyn AlertIncidentLifecycleSink>>,
}
...
impl RuleEvaluator {
    ...
    pub fn with_lifecycle_sink(mut self, sink: Arc<dyn AlertIncidentLifecycleSink>) -> Self {
    ...
    async fn eval_one(&self, rule: &AlertRule, now: TimestampMicros) -> Result<()> {
        ...
        if firing {
            match existing.as_ref() {
                ...
                Some(inc) if inc.status == IncidentStatus::Open => {
                    ...
                    if new_sev != inc.severity {
                        ...
                        let incident = self.incidents.update(updated).await?;
                ...
                && inc.status == IncidentStatus::Open
            {
                ...
                emit_lifecycle(self.lifecycle_sink.as_ref(), &incident).await;
                enqueue_notify_event(self.notify_engine.as_ref(), &incident, ALERT_RESOLVED_EVENT)
                    .await;
