...
//!
//! 失败策略：单步失败 → drop 该 event 进 `IntakeResult.errors`，整 batch 不回滚；
//! 这与 schema 校验失败的处理一致。
...
pub use crate::domain::function::FunctionExecutor;
use crate::{
    domain::{
        function::{Function, FunctionRepository},
        intake::{IntakeBatch, IntakeError},
...
impl PipelineEngine {
    ...
    pub async fn apply(&self, batch: &mut IntakeBatch) -> Result<Vec<IntakeError>> {
        let pipelines = self
            .pipelines
