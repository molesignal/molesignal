...
//!
//! 后台 loop（intake flush / compactor / alert_manager）已在 [`crate::bootstrap::build_state`]
//! 里按角色 gate spawn；本模块只负责前台 server：
//! - `Standalone` → [`http_server::serve`]（HTTP + gRPC 同进程）
//! - `Router` → [`router::run`]（反代 HTTP）
//! - `Intake` / `Querier` → gRPC（intake + Arrow Flight scan + cluster RPC）
//!
//! `Standalone` 等价于全角色。多角色节点（如 `["intake","querier"]`）会起一份去重后的
...
pub fn plan_servers(roles: &[Role]) -> ServerPlan {
    ...
    // intake / querier 对外暴露 gRPC（intake push、Flight scan shard、cluster）。
    let grpc = has(Role::Intake) || has(Role::Querier);
...
pub async fn run(state: AppState, settings: &Settings) -> anyhow::Result<()> {
    ...
    // 对外 OTLP gRPC（traces/logs/metrics/profiles）：intake 角色节点按配置起独立 listener。
    // `Full`（standalone）路径由 http_server::serve 内部接管，这里跳过避免双绑端口。
    if plan.http != HttpKind::Full && settings.node.roles.contains(&Role::Intake) {
        let s = state.clone();
        let cfg = settings.otlp_grpc.clone();
        set.spawn(async move { crate::api::grpc::serve_otlp_grpc(s, &cfg).await });
    }
    // Probe 控制面与告警/自动发布职责同节点部署；standalone 已由 full server 接管。
    if plan.http != HttpKind::Full && settings.node.roles.contains(&Role::AlertManager) {
        let s = state.clone();
        let cfg = settings.probe.clone();
        set.spawn(async move { crate::api::grpc::serve_probe_grpc(s, &cfg).await });
    }
    ...
    // 优雅退役：SIGTERM/SIGINT → begin_drain → 等 intake flush 干净（drained）后退出。
    ...
    // server 的纯后台角色也靠它保活（替代 keepalive），收到信号才退出。
    {
        ...
        let has_intake = has(Role::Intake);
...
/// 关停看守：等首个 SIGTERM/SIGINT → 触发 drain → 等节点 drained（仅 intake 节点有待 flush
...
async fn run_shutdown(
    ...
    drain.begin_drain();
    if !has_intake {
        // 无 intake（纯 querier/router/compactor）：无待 flush 数据；drain 标记 + heartbeat
...
mod tests {
    ...
    #[test]
    fn intake_querier_share_one_grpc_no_http() {
        assert_eq!(
            plan_servers(&[Role::Intake, Role::Querier]),
            ServerPlan {
                http: HttpKind::None,
                grpc: true
            }
