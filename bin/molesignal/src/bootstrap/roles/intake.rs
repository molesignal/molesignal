...
//! Intake role：IntakeWorker 组合 DurableIntakeSink（WAL→buffer 串行）+ 后台 flush
//! scheduler（buffer→parquet→parquet_file_meta→wal_truncate 四步）+ 启动 replay。
//!
//! 由 [`crate::bootstrap::build_state`] 构造一个 `Arc<IntakeWorker>` 后：
...
//!    遍历 BufferPool 触发 [`IntakeWorker::flush_one`]。
//! 3. `Arc<IntakeWorker>` 同时是 `IntakeSink`，注入到 [`IntakeService`]。
...
use crate::{
    ...
    infra::{
        ...
        intake::{
            AdaptiveRotation, BufferKey, BufferPool, DurableIntakeSink, FlushInflightGuard,
...
impl IntakeWorker {
    ...
    pub fn with_field_keys(mut self, svc: Arc<FieldKeyService>) -> Self {
        self.sink = Arc::new(
            DurableIntakeSink::new(self.wal.clone(), self.buffer.clone(), self.streams.clone())
    ...
    pub async fn recover_and_replay(&self) -> Result<()> {
        ...
        for (key, records) in restored {
            ...
            for rec in records {
                let batch: IntakeBatch = match serde_json::from_slice(&rec.payload) {
                    Ok(b) => b,
                    Err(e) => {
        ...
        tracing::info!(rows = row_count, "intake replay loaded; force-flushing");
    ...
    async fn flush_one_inner(&self, key: &BufferKey, due_at: Option<Instant>) -> Result<()> {
        ...
        let physical_stream = crate::infra::intake::physical_schema::project(&stream_def, key.3);
    ...
    async fn restore_buffer(
        &self,
        buf_handle: &Arc<tokio::sync::Mutex<crate::infra::intake::RecordBuilder>>,
    ...
    pub fn spawn_flush_loop(self: Arc<Self>) -> JoinHandle<()> {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                // 节点退役：写入已被 IntakeService 拒（停接新活），把残留 buffer 再 flush 一遍
...
mod tests {
    ...
    fn batch_of(s: &StreamDefinition, base_ts: i64, n: usize) -> IntakeBatch {
        IntakeBatch {
            batch_id: Id::new(),
