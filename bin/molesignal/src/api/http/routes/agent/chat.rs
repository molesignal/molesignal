...
//! Mole Agent chat HTTP 路由。
//!
//! - `GET    /api/v1/agent/chat`               列出当前用户活跃会话
//! - `POST   /api/v1/agent/chat`               新建会话
//! - `GET    /api/v1/agent/chat/{id}/messages` 拉历史
//! - `POST   /api/v1/agent/chat/{id}/messages` 发用户消息 + SSE 流回 Mole Agent
//! - `POST   /api/v1/agent/chat/{id}/archive`  归档 transcript 到对象存储
//! - `DELETE /api/v1/agent/chat/{id}`          归档后软删会话
//!
//! Mole Agent 分析能力：
//! - provider 从 PG `agent_model_providers` 行 + 加密 key 构造 adapter（dev 缺省 env fallback）。
...
pub fn routes() -> Router<AppState> {
    Router::new()
        .route("/agent/chat", get(list_chats).post(create_chat))
        .route(
            "/agent/chat/{id}/messages",
            get(list_messages).post(post_message),
        )
        .route(
            "/agent/chat/{id}/archive",
...
async fn create_chat(
    ...
    Ok(Json(to_chat_resp(state.agent.chats.create_chat(s).await?)))
}
...
async fn delete_chat(
    ...
    state.agent.chats.delete_chat(&ctx.org_id, &Id(id)).await?;
...
async fn list_messages(
    ...
    let _ = state
        ...
        .get_chat(&ctx.org_id, &Id(id.clone()))
        .await?;
    let msgs = state.agent.chats.list_messages(&Id(id)).await?;
    Ok(Json(json!({
...
async fn post_message(
    ...
    let history_rows = if let Some(message_id) = req.regenerate_from_message_id.as_deref() {
        regeneration_history(
            state.agent.chats.list_messages(&chat.id).await?,
            message_id,
            &req.content,
        )?
    } else {
    ...
    //    错误 / tool evidence，流结束后落 assistant row + 审计 + agent_model_traces。
    ...
    crate::shared::trace_context::spawn_with_current_trace_context(async move {
        ...
        // 8) agent_model_traces event（best-effort）。
        ...
        let _ = persist_state
            .intake
            .intake_internal_telemetry(IntakeBatch {
                batch_id: Id::new(),
...
fn prompt_readable(
    t: &crate::infra::persistence::repositories::agent::prompts::AgentPromptTemplate,
    ctx: &IamContext,
) -> bool {
...
async fn resolve_prompts(
    ...
    let task = if dashboard_capability_active {
        ...
        match state.agent.prompts.get(&Id(pid.to_string())).await {
...
async fn archive_chat(
    ...
    ctx: &IamContext,
    chat: &Chat,
) -> crate::infra::persistence::repositories::agent::chat_archives::ChatArchive {
    use crate::infra::persistence::repositories::agent::chat_archives::ChatArchive;
