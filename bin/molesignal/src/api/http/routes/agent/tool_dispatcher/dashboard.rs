...
pub(super) async fn get_capabilities(
    ...
    result_json(&state.agent.dashboard_authoring.capabilities().await?)
}
...
fn require_authoring_access(auth: &IamContext) -> Result<()> {
    Permission::require_key(auth, "agent.use")?;
...
mod tests {
    ...
    #[test]
    fn dashboard_authoring_requires_agent_and_dashboard_permissions() {
        assert!(require_authoring_access(&auth_with(&["agent.use"])).is_err());
        assert!(require_authoring_access(&auth_with(&["dashboards.create"])).is_err());
        assert!(require_authoring_access(&auth_with(&["agent.use", "dashboards.create"])).is_ok());
    }
