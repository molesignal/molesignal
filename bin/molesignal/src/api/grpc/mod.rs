...
use crate::{
    ...
    config::{FlightSqlSettings, GrpcSettings, OtlpGrpcSettings, ProbeGrpcSettings},
    infra::persistence::repositories::cluster::nodes::ClusterNodesRepository,
};

/// Probe Agent 双端口控制面：注册入口使用服务端 TLS；Control 使用同一 CA 的 mTLS。
pub async fn serve_probe_grpc(state: AppState, settings: &ProbeGrpcSettings) -> anyhow::Result<()> {
    let control = state
        .probe_control
        .clone()
        .ok_or_else(|| anyhow::anyhow!("Probe control service is not initialized"))?;
    let tls = control.server_tls();
    let register_addr: SocketAddr =
        format!("{}:{}", settings.bind, settings.register_port).parse()?;
    ...
    let register_server = async move {
        tracing::info!(addr = %register_addr, "Probe registration gRPC server listening");
        Server::builder()
            .tls_config(register_tls)?
            .layer(trace::layer::GrpcTraceLayer)
            .add_service(register_service)
            .serve(register_addr)
            .await
    ...
    let control_tls = ServerTlsConfig::new()
        ...
        .client_ca_root(Certificate::from_pem(tls.ca_certificate_pem));
    let control_server = async move {
        tracing::info!(addr = %control_addr, "Probe control mTLS gRPC server listening");
        Server::builder()
            ...
            .serve(control_addr)
            .await
            .map_err(|error| anyhow::anyhow!("Probe control gRPC serve: {error}"))
    };

    tokio::try_join!(register_server, control_server)?;
    Ok(())
}
