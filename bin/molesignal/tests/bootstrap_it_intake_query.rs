...
#[tokio::test]
async fn intake_api_accepts_batch_and_query_api_returns_seeded_rows() {
    ...
    // 1. POST intake（3 条 + 1 个新字段 `latency_ms` 触发 schema 演化）
    let resp = s
        .client
        .post(format!("{}/api/v1/intake/logs/app", s.base_url))
...
/// 撤掉启动期预 seed 后，写入一个不存在的流应由 `IntakeService` 用推断 schema
/// 自动建流（schema-on-write），随后照常 flush + 可查。
#[tokio::test]
async fn intake_auto_creates_missing_stream() {
    if skip_unless_enabled() {
        eprintln!("skipped (set MS_RUN_IT=1 to enable)");
        return;
    }
    ...
    let resp = s
        .client
        .post(format!("{}/api/v1/intake/logs/auto_created", s.base_url))
        .header(s.auth_header().0, s.auth_header().1)
        .json(&json!([
