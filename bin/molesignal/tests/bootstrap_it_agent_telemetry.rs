...
//! Mole Agent 模型遥测 license-gate 冒烟。
//!
//!  feature 启用 + license OFF → 4 个 agent 路由全返 403。
//! 实际 license ON 路径由 it_agent_chat / it_agent_mcp 覆盖（依赖 wiremock 假 LLM）；
//! 本文件只关心 gate 是否生效，跑得最快。

mod common;

#[tokio::test]
async fn agent_routes_blocked_when_license_off() {
    ...
    // OSS 默认 CommunityLicense → has_feature("agent") == false → 路由返 403
    let routes = [
        ("POST", "/api/v1/agent/chat"),
        ("GET", "/api/v1/agent/chat"),
        ("POST", "/api/v1/agent/chat/chat-id/messages"),
        ("GET", "/api/v1/agent/stats"),
        ("GET", "/api/v1/agent/dashboard-authoring/capabilities"),
    ];
