...
//!
//! ```bash
//! MS_RUN_IT=1 cargo test -p molesignal --test infra_it_agent -- --nocapture
//! ```
...
async fn provider_crud_and_encrypted_key_roundtrip() {
    ...
    // 2. 密文落库 != 明文。
    let row =
        sqlx::query("SELECT ciphertext FROM agent_model_provider_secrets WHERE provider_id = $1")
            .bind(&id.0)
...
async fn audit_query_filters_and_cursor_stability() {
    ...
    for (i, (action, tk)) in [
        ...
        ("agent.chat.archived", "agent_chat"),
    ]
    .iter()
    .enumerate()
    {
