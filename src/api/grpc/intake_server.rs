...
//! `intake.v1.IntakeService` 的 server 端实现。
//!
//! payload 约定：`serde_json::to_vec(&Vec<RawEvent>)`（client 把整批事件序列化为
//! JSON 数组；server 反序列化后包成 `IntakeBatch` 调 [`IntakeService::intake`]）。
//!
...
use crate::{
    ...
    protocol::intake::v1::{
        IntakeError as ProtoIntakeError, PushRequest, PushResponse, StreamType as ProtoStreamType,
        intake_service_server::{IntakeService as IntakeServiceTrait, IntakeServiceServer},
    },
    shared::{ids::Id, time::TimestampMicros},
};

pub struct IntakeGrpc {
    service: Arc<IntakeService>,
    self_telemetry_token: Option<Arc<str>>,
}

impl IntakeGrpc {
    pub fn new(service: Arc<IntakeService>) -> Self {
        Self {
            service,
    ...
    pub fn into_server(self) -> IntakeServiceServer<Self> {
        IntakeServiceServer::new(self)
    }
}

#[tonic::async_trait]
impl IntakeServiceTrait for IntakeGrpc {
    async fn push(&self, request: Request<PushRequest>) -> Result<Response<PushResponse>, Status> {
        ...
        let result = if internal_self_telemetry {
            self.service.intake_self_telemetry(batch).await
        } else {
            self.service.intake(batch).await
        }
...
fn error_to_status(error: crate::shared::Error) -> Status {
    match error {
        ...
        other => Status::internal(format!("intake: {other}")),
    }
}
...
mod tests {
    ...
    #[test]
    fn ordinary_grpc_intake_does_not_gain_internal_privileges() {
        assert!(
