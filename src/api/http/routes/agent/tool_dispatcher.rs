...
//!
//! 每次调用同步写入 `agent_tool_calls`；审计落库失败时调用失败关闭。
...
/// 用户分析设计的（如 `agent_model_traces` 的 cost_usd 等），会导致聚合查询失败 → 重试循环。
fn is_internal_stream(name: &str) -> bool {
    name == "_agent_model_traces" || name.starts_with('_')
}
...
async fn handle_list_streams(
    ...
    let candidates: Vec<_> = filter_streams(all, args.stream_type)
        ...
        .collect();
    // 不可查询的 stream（仅作 intake 入口 / pipeline 源）对 Mole Agent 没有意义：排除掉，免得模型
