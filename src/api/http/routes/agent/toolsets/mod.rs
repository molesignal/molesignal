    ...
    pub fn mcp_tool(&self, name: &str) -> Option<&McpTool> {
        self.mcp_tools.get(name).filter(|tool| {
            self.network_access == NetworkAccess::Allowed
                && tool.enabled
                && tool.status == crate::agent::tool_control::ManagedToolStatus::Healthy
...
pub async fn resolve_toolsets_for_profile(
    ...
    let toolsets = state.agent.toolsets.list(org_id).await?;
    ...
    let profiles = state.agent.repository.list_profiles(org_id).await?;
