...
VALUES
    ...
    ('molesignal-system-overview', 13, 'overview-warning-error-logs', 'Warnings and errors', 'Recent warning and error events emitted by MoleSignal.', 0, 72, 24, 24, 'logs', NULL, NULL, NULL, NULL, $queries$[{"refId":"A","enabled":true,"dataSourceType":"logs","query":{"language":"sql","expression":"SELECT * FROM _molesignal WHERE level IN ('WARN', 'ERROR') ORDER BY _timestamp DESC LIMIT 100","streamName":"_molesignal","streamType":"logs","limit":100}}]$queries$::JSONB)
),
metric_panels AS (
    SELECT
