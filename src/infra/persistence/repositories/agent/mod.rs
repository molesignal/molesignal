...
//! Mole Agent 调查、自动化、审批、执行与 Agent Profile 的 Postgres 实现。
...
use crate::{
    agent::model::{
        AgentProfile, AgentRepository, ApprovalRequest, ApprovalStatus, Automation, Execution,
        Investigation, InvestigationDetail, InvestigationEvidence, InvestigationHypothesis,
        InvestigationStep, NetworkAccess, ToolCallRecord,
    },
...
impl AgentRepository for PgAgentRepository {
    ...
    async fn get_investigation(&self, org_id: &Id, id: &Id) -> Result<InvestigationDetail> {
        let investigation =
            sqlx::query("SELECT * FROM agent_investigations WHERE org_id = $1 AND id = $2")
                .bind(&org_id.0)
                .bind(&id.0)
        ...
        let steps = sqlx::query(
            "SELECT * FROM agent_investigation_steps
        ...
        let evidence = sqlx::query(
            "SELECT * FROM agent_investigation_evidence
        ...
        let hypotheses = sqlx::query(
            "SELECT * FROM agent_investigation_hypotheses
    ...
    async fn append_step(&self, item: InvestigationStep) -> Result<InvestigationStep> {
        sqlx::query(
            "INSERT INTO agent_investigation_steps
    ...
    async fn append_evidence(&self, item: InvestigationEvidence) -> Result<InvestigationEvidence> {
        sqlx::query(
            "INSERT INTO agent_investigation_evidence
    ...
    async fn upsert_hypothesis(
        ...
        sqlx::query(
            "INSERT INTO agent_investigation_hypotheses
             ...
             WHERE agent_investigation_hypotheses.org_id = EXCLUDED.org_id",
        )
        .bind(&item.id.0)
        .bind(&item.investigation_id.0)
        .bind(&item.org_id.0)
    ...
    async fn get_automation(&self, org_id: &Id, id: &Id) -> Result<Automation> {
        sqlx::query("SELECT * FROM agent_automations WHERE org_id=$1 AND id=$2")
            .bind(&org_id.0)
            .bind(&id.0)
            .fetch_optional(&self.pool)
            .await
    ...
    async fn get_approval(&self, org_id: &Id, id: &Id) -> Result<ApprovalRequest> {
        sqlx::query("SELECT * FROM agent_approval_requests WHERE org_id=$1 AND id=$2")
            .bind(&org_id.0)
            .bind(&id.0)
            .fetch_optional(&self.pool)
            .await
    ...
    #[tracing::instrument(
        name = "db.transaction",
        skip_all,
        fields(db.system.name = "postgresql", db.operation.name = "TRANSACTION", db.collection.name = "agent_approval_requests")
    )]
    ...
    async fn get_execution(&self, org_id: &Id, id: &Id) -> Result<Execution> {
        sqlx::query("SELECT * FROM agent_executions WHERE org_id=$1 AND id=$2")
            .bind(&org_id.0)
            .bind(&id.0)
            .fetch_optional(&self.pool)
            .await
    ...
    async fn find_execution_by_key(&self, org_id: &Id, key: &str) -> Result<Option<Execution>> {
        sqlx::query("SELECT * FROM agent_executions WHERE org_id=$1 AND idempotency_key=$2")
            .bind(&org_id.0)
            .bind(key)
            .fetch_optional(&self.pool)
            .await
    ...
    async fn get_profile(&self, org_id: &Id, id: &Id) -> Result<AgentProfile> {
        sqlx::query("SELECT * FROM agent_profiles WHERE org_id=$1 AND id=$2")
            .bind(&org_id.0)
            .bind(&id.0)
            .fetch_optional(&self.pool)
            .await
    ...
    #[tracing::instrument(
        name = "db.transaction",
        skip_all,
        fields(db.system.name = "postgresql", db.operation.name = "TRANSACTION", db.collection.name = "agent_profiles")
    )]
    async fn create_profile(&self, item: AgentProfile) -> Result<AgentProfile> {
        let mut tx = sqlx::begin(&self.pool).await.map_err(sqlx_err)?;
        if item.is_default {
            sqlx::query(
                "UPDATE agent_profiles SET is_default=FALSE
