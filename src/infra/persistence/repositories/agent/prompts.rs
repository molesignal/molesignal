...
//! `agent_prompt_templates` 表 Pg 实装。
//!
...
impl AgentPromptRepository for PgAgentPromptRepository {
    async fn list(&self, org_id: &Id, user_id: &Id) -> Result<Vec<AgentPromptTemplate>> {
        let rows = sqlx::query(&format!(
            "SELECT {COLS} FROM agent_prompt_templates
             WHERE scope = 'builtin'
    ...
    async fn get(&self, id: &Id) -> Result<AgentPromptTemplate> {
        let row = sqlx::query(&format!(
            "SELECT {COLS} FROM agent_prompt_templates WHERE id = $1"
        ))
        .bind(&id.0)
    ...
    #[tracing::instrument(
        name = "db.transaction",
        skip_all,
        fields(db.system.name = "postgresql", db.operation.name = "TRANSACTION", db.collection.name = "agent_prompt_templates")
    )]
    async fn set_default(&self, id: &Id) -> Result<()> {
        ...
        sqlx::query(
            "UPDATE agent_prompt_templates SET is_default = FALSE, updated_at_micros = $5
        ...
        .map_err(sqlx_err)?;
        sqlx::query(
            "UPDATE agent_prompt_templates SET is_default = TRUE, enabled = TRUE, updated_at_micros = $2
             WHERE id = $1",
        )
        .bind(&id.0)
        .bind(now.0)
    ...
    async fn delete(&self, id: &Id) -> Result<()> {
        ...
        sqlx::query("DELETE FROM agent_prompt_templates WHERE id = $1")
            .bind(&id.0)
            .execute(&self.pool)
            .await
            .map_err(sqlx_err)?;
