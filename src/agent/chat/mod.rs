...
//!   结果回灌 LLM → 最终 assistant 回答；全程写 `agent_model_traces`
//!
...
use crate::{
    agent::tools::{ToolAuthContext, ToolCall, ToolDispatcher, ToolResult},
...
pub trait ProviderAdapter: Send + Sync {
    ...
    /// Streaming variant（change `agent-provider-adapter`）：返一个 SSE 帧拆解后的
...
/// （task 3.3 / design D5）。
///
/// 约定：每个 provider 用 `MS_AGENT_<UPPER>_API_KEY` 与 `_BASE_URL` 两个
/// env var 注入；缺 API key → `Err(invalid)`。
pub fn adapter_from_env(provider: Provider) -> Result<Arc<dyn ProviderAdapter>> {
    let prefix = match provider {
        Provider::OpenAi => "MS_AGENT_OPENAI",
        Provider::Anthropic => "MS_AGENT_ANTHROPIC",
        Provider::OpenAiCompatible => "MS_AGENT_COMPATIBLE",
    };
    match provider {
...
fn tool_failure_result(error: Error) -> ToolResult {
    let content = match error {
        Error::Validation { message, issues } => crate::agent::tools::ToolContent::Json {
        ...
        other => crate::agent::tools::ToolContent::Text {
...
impl AgentLoop {
    ...
    /// Streaming run（change `agent-provider-adapter` 2.x）：把 provider 增量
